...
setwd("~/BINF/yushi scrnaseq/E10.5/sox9/1-sox9-10-5_clean_RSEC_MolsPerCell_MEX")
sox9data = ReadMtx(mtx = "matrix.mtx.gz",
                   cells = "barcodes.tsv.gz",
                   features = "features.tsv.gz")
setwd("~/BINF/yushi scrnaseq/E10.5/sox9/seurat output")
...
sox9 <- subset(sox9, subset = nFeature_RNA > 2000 & nFeature_RNA < 7500 & percent.mt < 5)
...
#chose 13
sox9 = FindNeighbors(sox9, dims = 1:13)
...
#likely clusters 3 and 4
#res .3 chosen.

DimPlot(sox9, reduction = "umap", label = TRUE,
        group.by = "RNA_snn_res.0.3")
#nc markers likely 4,3,8. may be also 10 and 11
sox.3 = SetIdent(sox9, value = "RNA_snn_res.0.3")
sox.3 = AddModuleScore(sox.3, features = migratory.markers)
...
sox_3_markers <- read_excel("filtmarkers_resolution_0.3.xlsx")
sox_3_markers = as.data.frame(sox_3_markers)
res3 <- gptcelltype(sox_3_markers, tissuename = 'mouse embryo', 
                    model = 'gpt-4')
'
new.cluster.ids <- c("Mesoderm Cells",
                     "Neural Crest Cells",
                     "Mesenchymal Cells",
                     "Craniofacial Mesenchyme Cells",
                     "Neural Crest Cells",
                     "Neural Progenitor Cells",
                     "Forebrain Progenitor Cells",
                     "Mesenchyme Cells",
                     "Epithelial Cells",
                     "Endothelial Cells",
                     "Neurons",
...
new.cluster.ids <- c("Sclerotome cells",
...
sox9 = SetIdent(sox9, value = "RNA_snn_res.0.3")
